...
filename = 'keyboard27.json'
file = open (filename, "r", encoding="UTF-8")
keyboard27=file.read()
...
for event in longpoll.listen(): #проверка действий
    ...
    if event.type == VkBotEventType.MESSAGE_NEW:
        if(lastMessage == "Регистрация" and text != "Регистрация"):
            ...
            reply = "Введите в свободном формате номер телефона и электронную почту, куда будет выслан электронный сертификат участника. ";
            lastMessage = "Электронная почта";
            print(text);
            keyboard = empty_keyboard;
        elif(lastMessage == "Электронная почта"):
            print(text);
            reply = "Завершающие элемент регистрация. Дайте согласие на обработку своих персональных данных.\nЕсли Вам нет 18 лет, то пришлите фото или скан заявки от родителей."
            keyboard = keyboard27
        ...
        while text == "Начать":
            reply = "Ознакомьтесь с положением проекта, контактами и сайтом. Не забудьте подписаться на группу Вконтакте РМЦ.\nПроект предоставляет возможность пройти два этапа в течение 24-30 августа. После 30 августа результаты не принимаются. КВИЗ займет примерно 25 минут времени. Если прохождение КВИЗа превысит лимит времени в 40 минут, то результаты аннулируются, поэтому найдите свободное время и настройтесь на прохождение тестовых заданий.\nКогда Вы будите готовы к прохождению первого этапа нажмите кнопку \"Регистрация\""
            keyboard = keyboard2
            break
        ...
        if text == "Знакомство с положением проекта":
            ...
            reply = "Вас приветствует цифровой помощник военно-спортивной игры \"В отрыв\"\n!Нажмите кнопку \"Начать\" для прохождения испытания в интеллектуальном КВИЗе, а также для зачисления результатов беговой дисциплины в 3 км.\nПризовой фонд для победителей составляет 90 000 рублей."
            keyboard = keyboard1
        elif text == "Контакты":
            ...
            reply = 'Распечатайте следующую заявку, заполните и отправьте цифровому помощнику для завершению регистрации.'
            keyboard = empty_keyboard
            # todo добавить распечатку
            lastMessage = "18 лет"
        elif lastMessage == "18 лет" or text == "Мне есть 18 лет":
            reply = "Вы успешно прошли регистрацию. После прохождения всех испытаний Ваше имя появится в таблице рейтинга с результатами на сайте www.вотрыв.рф 27 августа\n\nВы готовы приступить к выполнению КВИЗа?"
            keyboard = keyboard3
            lastMessage = " "
        elif(reply == " "):
